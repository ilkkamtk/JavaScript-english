...
* [List of open application programming interfaces](https://www.programmableweb.com/category/all/apis)
...
```
The ES8 version of JavaScript introduced the syntax of [async / await] (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/async_function) to simplify the use of Promise and especially error handling. With Async / await syntax, functions that return Promise are handled in much the same way as any other function. The difference is that the function that returns Promise must be written inside another, asynchronous (async) function. In addition, await is written in front of the function call. Here's an earlier example using async / await syntax, but now with error handling.
```html
<figure>
    <img>
    <figcaption></figcaption>
